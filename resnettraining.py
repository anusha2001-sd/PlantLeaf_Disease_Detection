...
def process(path):
    ...
    decay_rate=0.9)
        ...
        plt.xlabel('epoch')
        plt.legend(['train', 'Validation'], loc='upper left')
        plt.savefig('results/Resnet_Training and validation Loss.png') 
        plt.pause(5)
        plt.show(block=False)
        plt.close()
        model.save('Leafdisease_resnet.h5')
        ...
        cm_display.plot()
        plt.show()
        plt.savefig("results/Resnet_CM.png")
        fpr = {}
        tpr = {}
        thresh ={}
        n_class = 3
        ...
        plt.legend(loc='best')
        plt.savefig('results/Resnetnet-Multiclass ROC',dpi=300);
        plt.show() 
